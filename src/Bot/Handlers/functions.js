  ...
  client.reactPrompt = async (message, question, opt) => {
    if (!opt) {
      ...
      const filter = (reaction, user) =>
        [client.emoji.checkMark, client.emoji.redX].includes(
          reaction.emoji.name
...
/*  client.clearSongQueue = () => {
  ...
  client.raidModeActivate = async guild => {
    ...
    const perms = new Discord.Permissions(everyone.permissions).remove(
      "SEND_MESSAGES"
    );
...
Would you like to ban all ${client.raidJoins.length} members that joined in the raid?`);
    ...
    const filter = (reaction, user) =>
      ...
      guild.members.fetch(user).then(m => m.hasPermission("BAN_MEMBERS")) &&
      !user.bot;
    client.raidMessage
      ...
      .then(async collected => {
        ...
        if (reaction.emoji.name === client.emoji.checkMark) {
          ...
          client.success(
            staffChat,
            "Banning!",
            `User ${modUser.tag} has chosen to ban the raid. It may take some time to finish banning all raid members.`
          );
          ...
          const interval = setInterval(() => {
            if (client.raidJoins.length !== 0) {
              ...
              client.raidJoins
                .shift()
                .ban({ days: 1, reason: "Member of raid." })
                .catch(console.error);
            } else {
              // We've finished banning, annouce that raid mode is ending.
              staffChat.send(
                "Finished banning all raid members. Raid Mode is deactivated."
              );
              joinLeaveLog.send(
                `The above ${client.raidMembersPrinted} members have been banned.`
              );
          ...
          client.error(
            staffChat,
            "Not Banning!",
            `User ${modUser.tag} has chosen to not ban the raid. Raid Mode is deactivated.`
          );
          // Reset all raid variables
          client.raidMode = false;
          client.raidJoins = [];
