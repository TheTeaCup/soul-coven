...
async function loadIntoMemory(client, guildID, channelID, limit) {
  ...
  while (messagesLeft) {
    if (messagesLeft > 100) {
      ...
      console.log(`${messagesLeft} messages left to load`);
      let moreMessages = await channel.messages.cache.fetch({
        limit: 100,
  ...
  const newPostsMap = newPosts.reduce((mapAccumulator, obj) => {
    ...
    // mapAccumulator[obj.key] = obj.val;
    mapAccumulator[
      String(obj.embeds[0].footer.text).match(/\((\d{18})\)/)[1]
    ] = {
      p: true,
      lc: settings.threshold + 1,
      legacy: false,
  ...
  console.log(
    `Loaded ${Object.keys(postsMap).length} legacy posts, and ${
      Object.keys(newPostsMap).length
    } new posts in ${settings.reactionEmoji} channel`
  );
  ...
  msgChannel.messages.fetch(msg.id).then(msg => {
    ...
    msg.reactions.cache.forEach(reaction => {
      if (reaction.emoji.name == settings.reactionEmoji) {
        console.log(
          `message ${settings.reactionEmoji}'d! (${msgID}) in #${msgChannel.name} total: ${reaction.count}`
        );
        ...
        if (reaction.count >= settings.threshold) {
          ...
          if (messagePosted[msgID].hasOwnProperty("psm")) {
            ...
            console.log(
              `posting message with content ID ${msgID}. reaction count: ${reaction.count}`
            );
