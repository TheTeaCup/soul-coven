...
module.exports.run = async (client, message, args) => {
  ...
  if (!deleteCount || deleteCount < 1 || deleteCount > 100) {
    return client.error(
      message.channel,
      "Invalid Number of Messages to Purge!",
      "Please provide a number **between 1 and 100** for the number of messages to delete!"
    );
  }

  const descision = await client.reactPrompt(
    message,
    `Would you like to delete ${deleteCount} messages from <#${message.channel.id}>?`
  );
    ...
    return message.channel
      ...
      .catch(error =>
        client.error(
          message.channel,
          "Purge Failed!",
          `Couldn't delete messages because: \`${error}\``
        )
      );
  }
  return client.error(
    message.channel,
    "Messages Not Purged!",
    "The prompt timed out, or you selected no."
  );
};
...
module.exports.help = {
  ...
  usage: "purge <number>",
  details:
    "<number> => From 1-100. I wish it could be more but Discord's API doesn't allow that..."
};
